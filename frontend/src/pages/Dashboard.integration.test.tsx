...
describe("Dashboard integration", () => {
  ...
  it("uses per-camera ONVIF credentials when resolving and testing a selected camera", async () => {
    ...
    vi.mocked(discoverOnvifDevices).mockResolvedValue([
      {
        ...
        hardware: "hw-e",
        location: "Lobby",
        services: { media: "http://192.168.1.30/onvif/media" },
        xaddrs: "http://192.168.1.30/onvif/device_service",
      },
    ] satisfies ONVIFDevice[]);
    vi.mocked(resolveOnvifStreams).mockImplementation(async ({ device, username, password }) => {
      expect(username).toBe("front-user");
      expect(password).toBe("front-pass");
    ...
    fireEvent.change(passwordField, { target: { value: "front-pass" } });

    fireEvent.click(screen.getByRole("button", { name: "Resolve Streams" }));
    await waitFor(() => expect(resolveOnvifStreams).toHaveBeenCalledTimes(1));
    expect(resolveOnvifStreams).toHaveBeenCalledWith(
      expect.objectContaining({ username: "front-user", password: "front-pass" }),
    );

    fireEvent.click(screen.getByRole("button", { name: "Test Camera" }));
    await waitFor(() => expect(testOnvifCamera).toHaveBeenCalledTimes(1));
