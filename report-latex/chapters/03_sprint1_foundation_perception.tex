...
The use case is centered on the manager configuring a source and zone, then launching continuous analysis. For each frame, the system detects persons, associates identities, evaluates zone membership, and updates the queue state. This sequence is the basis for all later analytics and automation layers.
...
This implementation produced a stable perception baseline for later queue analytics and automation integration \cite{opencv_docs}.

\section{Conclusion}
Sprint 1 delivered the operational perception core: reliable person detection, ID tracking continuity, and queue-zone filtering. This foundation directly enabled Sprint 2, where queue metrics and stability-aware logic were added on top of these stable perception outputs.
