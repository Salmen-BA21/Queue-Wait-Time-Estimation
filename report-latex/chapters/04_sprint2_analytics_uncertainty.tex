...
Sprint 2 shifted the system from counting people to describing queue dynamics. The analytical layer now derives arrival and service rates, computes wait time, and marks when the queue should be treated as unstable.
...
In the queue-analysis module, arrivals and departures are tracked as timestamp deques over sliding windows. Let $N_a$ and $N_d$ be events in windows $T_a$ and $T_d$:
\[
...
\section{Conclusion}
Sprint 2 delivered the analytical core of the project: event-to-rate modeling, stable and unstable wait-time semantics, and a runtime metric contract that downstream components can consume without guessing.
