...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|}
        ...
        UC-4.1 & As a developer, I want to implement webhook dispatch with validated alert payloads and proper authorization so that alerts securely reach the notification workflow. & Define the webhook payload schema, validate it before dispatch, and protect delivery with shared-secret authorization. \\ \hline
        UC-4.2 & As a manager, I want Telegram notifications routed to the on-site operator so that field action can be triggered through a familiar channel. & Create and configure the Telegram bot, store its credential in n8n, and configure the workflow branches. \\ \hline
        UC-4.3 & As a developer, I want to implement resilient webhook delivery with retries, cooldown deduplication, and audit logging so that alert delivery is reliable and traceable. & Add retry logic, timeout handling, cooldown checks, and delivery logging for webhook notifications. \\ \hline
        UC-4.4 & As a developer, I want alert routing validated under realistic scenarios so that notification behavior is trustworthy before deployment. & Prepare PowerShell validation scripts for single-alert, multi-feed, and execution polling scenarios. \\ \hline
        UC-4.5 & As a developer, I want the complete monitoring chain tested and runtime behavior tuned so that the platform is reliable under realistic conditions. & Run backend and frontend integration checks for lifecycle actions, streaming, API contracts, and fallback behavior; apply configurable image size, frame stride, async dispatch, playback pacing, worker supervision, frame handoff continuity, and WebSocket reconnection improvements. \\ \hline
    \end{tabularx}
...
Several runtime tuning options were added to improve responsiveness. Two key parameters control the inference behavior:

\begin{itemize}
    \item \textbf{Image size (\texttt{imgsz}):} Resolution of frames sent to YOLO. Larger images (640×640) improve accuracy but cost more GPU; smaller images (320×320) are faster but miss details.
    
    \item \textbf{Frame stride:} How many frames to skip between detections. Stride=1 processes every frame; stride=2 processes every second frame; stride=3 is fastest but may miss events.
\end{itemize}


Table~\ref{tab:sprint4_tuning_examples} summarizes the configurations evaluated during tuning and the measured impact on inference time, GPU load, and effective throughput.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|l|l|l|l|X|}
        \hline
        \textbf{imgsz} & \textbf{Stride} & \textbf{Inference Time} & \textbf{GPU Load} & \textbf{Effective FPS} & \textbf{Use Case} \\ \hline
        320 & 3 & 5 ms & 18\% & 28 & Fast processing, minimal VRAM usage \\ \hline
        512 & 2 & 7 ms & 24\% & 15 & Balanced accuracy/speed \textit{(default)} \\ \hline
        640 & 1 & 11 ms & 33\% & 13 & High-accuracy, all frames processed \\ \hline
    \end{tabularx}
    \caption{Runtime configuration variants and measured performance characteristics.}
    \label{tab:sprint4_tuning_examples}
    \endgroup
\end{table}
